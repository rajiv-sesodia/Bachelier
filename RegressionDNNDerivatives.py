...
class NeuralNetwork:
    ...
    def gradient(self, eta, X, Y, stdsc):
        ...
        dcdz, dyda_check = self.calc_dcdz(a, z, Y)   
        dyda_check_scaled = np.divide(dyda_check[0], np.sqrt(stdsc.var_))
        ...
        for m in range(X.shape[0]):
            ...
            dyda.append(np.divide(dy,dy2).dot(np.resize(dcda[m], (1, dcda.shape[1])))*0.5*M)
